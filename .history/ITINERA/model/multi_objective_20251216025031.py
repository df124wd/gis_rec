...
def run_nsga2_optimization(candidate_data: pd.DataFrame,
    ...
    for idx in best_solution:
        ...
        site_scores.append({
            ...
            'original_idx': int(row.get('_original_idx', idx)),
            'name': str(row.get('宗地坐落', f'地块{idx}')),
    ...
    # 取Top-K
    recommended = site_scores[:top_k]
    
    print(f"  方案内{len(best_solution)}个地块，取Top-{top_k}:")
    for i, site in enumerate(recommended):
        print(f"    {i+1}. {site['name'][:30]}... 总分={site['total_score']:.2f}")
    ...
    return {
        ...
        'selected_solution': {
            'sites': best_solution.tolist(),
            'fitness': pareto_fitness[best_idx].tolist()
        },
        'filtered_count': len(filtered_data),
        'original_count': len(candidate_data)
    }
...
if __name__ == '__main__':
    ...
    # 定义目标
    objectives = [
        {'name': '交通_便利评分(0-10)', 'maximize': True, 'weight_key': 'traffic'},
        {'name': '价格_万元/㎡', 'maximize': False, 'weight_key': 'price'},
        {'name': '宗地面积(平方米)', 'maximize': True, 'weight_key': 'area'},
        {'name': '生活_便利评分(0-10)', 'maximize': True, 'weight_key': 'amenity'},
    ]
    
    # 硬约束
    constraints = [
        {'field': '宗地坐落', 'operator': 'contains', 'value': '花都', 'original_text': '花都区'},
        {'field': '土地用途', 'operator': 'contains', 'value': '工业', 'original_text': '工业用地'},
    ]
    
    # 权重
    weights = {'traffic': 0.4, 'price': 0.25, 'area': 0.2, 'amenity': 0.15}
    ...
    print("\n========== 最终推荐 ==========")
    for i, site in enumerate(result['recommended_sites']):
        print(f"{i+1}. {site['name']}")
        print(f"   总分: {site['total_score']:.2f}")
