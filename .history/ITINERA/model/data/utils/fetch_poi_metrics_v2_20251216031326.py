...
def extract_station_name(name: str) -> str:
    """
    提取站点核心名称，去除出入口后缀
    例如：
    - "广州火车站A口" -> "广州火车站"
    ...
    import re
    # 去除常见的出入口后缀
    # 匹配模式：站名 + (A/B/C/D/E/F/1/2/3等)(口/出口/入口/出入口/进站口/出站口)
    patterns = [
        r'[-—]?[A-Fa-f1-9][口出入站].*$',  # A口、B出口、1号口等
        r'[-—]?[东南西北][口出入站].*$',    # 东口、南出口等
        r'[-—]?进站口.*$',
        r'[-—]?出站口.*$',
        r'[-—]?出入口.*$',
...
def get_pois_with_details(lon: float, lat: float, poi_type: str, radius: int,
    ...
    if dedupe_stations and all_pois:
        seen_stations = {}  # {核心站名: poi_info}
        deduped_pois = []
        
        for poi in all_pois:
            core_name = extract_station_name(poi["name"])
            if core_name not in seen_stations:
                seen_stations[core_name] = poi
                deduped_pois.append(poi)
            else:
                # 保留距离更近的
                existing = seen_stations[core_name]
...
def process_single_site(idx: int, row: pd.Series) -> SiteMetrics:
    ...
    for poi_name, poi_type, radius, category in POI_TYPES:
        ...
        # 对火车站、地铁站启用去重
        dedupe = poi_name in STATION_TYPES
