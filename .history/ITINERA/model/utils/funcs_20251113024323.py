...
def sample_items(A: list, B: list, selected_clusters: list, threshold: int = 900, keep_prob: float = 0.8, keep_ids: list = None):
    ...
    """
    # Normalize keep_ids to a list to avoid None checks
    ...
    # Prepare remaining scores and ensure valid non-negative probabilities
    remaining_scores = np.array([B[i] for i in remaining_indices], dtype=float)
    # Replace NaNs with zeros
    remaining_scores = np.nan_to_num(remaining_scores, nan=0.0)
    # Clamp negatives to zero to satisfy probability requirements
    remaining_scores = np.clip(remaining_scores, 0.0, None)

    # If remaining scores sum to zero or there are no remaining items, sample uniformly
    if len(remaining_indices) == 0:
        sampled_indices = []
    elif np.sum(remaining_scores) <= 0:
        ...
        # If non-zero probabilities are fewer than requested sample size, fallback to uniform sampling
        sample_size = int(len(remaining_indices) * keep_prob)
        if np.count_nonzero(normalized_scores) < sample_size:
            sampled_indices = np.random.choice(remaining_indices, size=sample_size, replace=False)
        else:
        # Sample based on normalized scores
            sampled_indices = np.random.choice(remaining_indices, size=sample_size, p=normalized_scores, replace=False)
